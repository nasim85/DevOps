param([string]$DropLocation="C:\RevenueCycle\R2016.5\Care",[string]$ENV="CI",[string] $FileList="Database\aaDeployment_Scripts\Accretive_UserManagementPostUpdateScript_168300.txt",[string] $ReportMailTo="nasimuddin@accretivehealth.com",[string] $MasterDeploy="0",[string] $buildNumber=(Get-Date -format 'yyyyMMdd'))
            ...
            if(Test-Path $MatertxtPath\$txtFile)
                {
                    ...
                    foreach($txtRow in (Get-Content $MatertxtPath\$txtFile))
                    {
                        ...
                        {
                             Write-Warning "Script file $ScriptName not found"
                             LogFileGen -SummaryTxt  "Script file $ScriptName not found"
                             $Global:objStatusList+=New-Object PsObject -property @{  ENV = $ENV; Client = $Client; ServerName="not found"; DBName="";Status="Failed";ScriptFile=$ScriptName;DeploymentTime=(Get-Date -format "MM/dd/yyyy HH:mm:ss");Details="Script file $ScriptName not found"}
                        }
