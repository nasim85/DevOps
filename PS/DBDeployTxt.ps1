param([string]$DropLocation="C:\RevenueCycle\R2016.5\Care",[string]$ENV="CI",[string] $FileList="Accretive_UserManagementPostUpdateScript_168300.txt",[string] $ReportMailTo="nasimuddin@accretivehealth.com",[string] $MasterDeploy="0",[string] $buildNumber=(Get-Date -format 'yyyyMMdd'),[string] $DropTxtPath="Database\aaDeployment_Scripts")


#Add-PSSnapin SqlServerCmdletSnapin100
#Add-PSSnapin SqlServerProviderSnapin100
<#
$DropLocation="C:\RevenueCycle\Releases\R2016.6\Care"
$ENV="CI"
$FileList="MasterTXT_2016.6.txt"
#$FileList="SD_983533_Deploy.txt|RCOA-SD-962316.Deploy.txt|RCOA_SD_972718_Authworkflow_deploy.txt|SD_964583_ServiceCode_Update_Deploy.txt"
#RCOA-SD-962316.Deploy.txt|RCOA_SD_972718_Authworkflow_deploy.txt|SD_964583_ServiceCode_Update_Deploy.txt
$ReportMailTo="nasimuddin@accretivehealth.com"
$BUILDNUMBER=(Get-Date -format 'yyyyMMdd')
$MasterDeploy="1"
$DropTxtPath="Database\aaDeployment_Scripts"
#>
$isSuccessded="1"
$onErrorExit=$False
$objStatusList=@()
...
@( 
   ...
   # Function for execute sql scripts. =======================================
        ...
        Function sendMail()
        {
		        ...
		        $a = @{Expression={$_.Env};Label="Env";width=6}, `
                     @{Expression={$_.Client};Label="Client";width=6}, `
                     @{Expression={$_.ServerName};Label="Server";width=16}, `
                     @{Expression={$_.DeploymentTime};Label="DeploymentTime";width=10}, `
                     @{Expression={$_.DBName};Label="DBName";width=13}, `
                     @{Expression={$_.Status};Label="Status";width=8}, `
                     @{Expression={$_.ScriptFile};Label="ScriptFile";width=30}, `
                     @{Expression={$_.Details};Label="Details";width=30}
        ...
        Write-Host "Sending Deployment report..."
				...
				$subject="DB Txt file Deployment report - $FileList"
                $head="<style>body{font-family:Arial; font-size:11px;} th, td {background-color: white; } table{background-color: gray;}</style>"
                $body="Hello Team,<br /> <br />Deployment report for txt file(s) $FileList<br /><br />"
                ...
                $Global:objStatusList|Format-Table -Wrap $a
                LogFileGen -SummaryTxt "`n"
                LogFileGen -SummaryTxt "`n`n================================== Deployment Summary =================================="
                $Global:objStatusList|Format-Table -Wrap|Out-File $LogFilePath -Append
...
if($MasterDeploy -eq "1")
{
    ...
    {
        if(Test-Path $DropLocation\$DropTxtPath\$MtxtFile)
        {
            $MatertxtPath=split-path $DropLocation\$DropTxtPath\$MtxtFile -parent
            Write-Host "======================================= Start Master TXT deployment ======================================================="
            Write-Host "Begin Master txt file :- '$MtxtFile' `n" -ForegroundColor Green
            LogFileGen -SummaryTxt "Begin Master txt file :- '$MtxtFile' `n"
            $mTextContent=Get-Content $MatertxtPath\$MtxtFile
            if($mTextContent.Count -ne 0)
            {
            foreach($txtFile in (Get-Content $MatertxtPath\$MtxtFile))
            {
            if(Test-Path $MatertxtPath\$txtFile)
                    ...
                    foreach($txtRow in (Get-Content $MatertxtPath\$txtFile))
                    {
                        ...
                        if(($EnvDetailList| Where-Object { $_.dbclass -eq $DBClass -and $_.CLIENT -eq $Client -and $_.DBNAME -like '*'+$DBName+'*' -and $_.Replication -in ('Pub','None')}))
                        ...
                        else
                            {
                                ...
                                $Global:objStatusList+=New-Object PsObject -property @{  ENV = $ENV; Client = $Client; ServerName="not found"; DBName="";Status="Failed";ScriptFile=$ScriptName;DeploymentTime=(Get-Date -format "MM/dd/yyyy HH:mm:ss");Details=$msg}
                                if($onErrorExit -eq $true)
                                    {
                                      throw $_.Exception
                                    }
                        ...
                        {
                             Write-Warning "SQL Script file $ScriptName not found"
                             LogFileGen -SummaryTxt  "SQL Script file $ScriptName not found"
                             $Global:objStatusList+=New-Object PsObject -property @{  ENV = $ENV; Client = $Client; ServerName="not found"; DBName="";Status="Failed";ScriptFile=$ScriptName;DeploymentTime=(Get-Date -format "MM/dd/yyyy HH:mm:ss");Details="SQL Script file $ScriptName not found"}
                             if($onErrorExit -eq $true)
                                {
                                  throw $_.Exception
                                }
                        ...
                        Write-Warning "End txt file $txtFile deployment"
                ...
                {
                    Write-Warning "txt file $txtFile not found."
                    LogFileGen -SummaryTxt  "txt file $txtFile not found."
                    $Global:objStatusList+=New-Object PsObject -property @{  ENV = $ENV; Client = ""; ServerName=""; DBName="";Status="Failed";ScriptFile=$txtFile;DeploymentTime=(Get-Date -format "MM/dd/yyyy HH:mm:ss");Details="txt file $txtFile not found"}
                    if($onErrorExit -eq $true)
                        {
                          throw $_.Exception
                        }
            ...
            {
                Write-Host "`n Master txt file '$MtxtFile' is Empty."
                LogFileGen -SummaryTxt  "`n Master txt file '$MtxtFile' is Empty."
            }
            Write-Host "`nEnd Master txt file '$MtxtFile' deployment" -ForegroundColor Green
            LogFileGen -SummaryTxt  "End Master txt file :- $MtxtFile`n`n"
            Write-Host "`n >>"
        }
        else
        {
            Write-Warning "`nMaster file '$MtxtFile' not found. please check if Master file exists."
            LogFileGen -SummaryTxt  "`nMaster file '$MtxtFile' not found. please check if Master file exists."
            $Global:objStatusList+=New-Object PsObject -property @{  ENV = $ENV; Client = ""; ServerName=""; DBName="";Status="Failed";ScriptFile=$MtxtFile;DeploymentTime=(Get-Date -format "MM/dd/yyyy HH:mm:ss");Details="Master txt file $MtxtFile not found"}
            if($onErrorExit -eq $true)
                {
                  throw $_.Exception
                }
...
else
{
    ...
    {
        if(Test-Path $DropLocation\$DropTxtPath\$txtFile)
        {
            $txtFilePath=split-path $DropLocation\$DropTxtPath\$txtFile -parent
            Write-Host "*********************************************** Start TXT deployment ***************************************************"
            Write-Host "Begin TXT file '$txtFile'`n" -ForegroundColor Green
            LogFileGen -SummaryTxt  "Begin TXT file $txtFile"
            foreach($txtRow in (Get-Content $DropLocation\$DropTxtPath\$txtFile))
            {
        ...
        {
            Write-Warning "`nTxt file '$txtFile' not found"
            LogFileGen -SummaryTxt "`nTxt file '$txtFile' not found"
            $Global:objStatusList+=New-Object PsObject -property @{  ENV = $ENV; Client = ""; ServerName=""; DBName="";Status="Failed";ScriptFile=$txtFile;DeploymentTime=(Get-Date -format "MM/dd/yyyy HH:mm:ss");Details="txt file $txtFile not found"}
            if($onErrorExit -eq $true)
                {
                  throw $_.Exception
  ...
  {
    throw "Deployment Error: Please check Deployment logs for more details."
  }
