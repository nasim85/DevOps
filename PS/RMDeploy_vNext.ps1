...
Param (
        ...
        # Environment on which deployement would be done...
        [Parameter(Mandatory=$true, Position=0)]
        [ValidateSet("UAT", "CI", "Prod")]
        [string]$Env = "CI",

                # Environment on which deployement would be done...
        [Parameter(Mandatory=$true, Position=1)]
        [string]$SharedPath = "\\ahv-tfsapp01\TFS_Drop\A2A\RM-Web-Test\Web.001"
)
#>

$ApplicationPathRoot="C:\Users\300915\Desktop\RMTool\RMWebDeploy"
#$BUILDNUMBER="Tran_DB-RM-Tran-R2016.2_20160208.1"
$ENV="CI"
#$DbList="15"
...
$DropLocation += "\Website"
$onErrorExit=$False


try
{
    Write-Verbose ("ENV=$ENV ,  buildNumber= $BUILDNUMBER , DropLocation=$DropLocation")
...
###################### Global Variables ########################################
 ...
 $LogFilePath =$LogPath + "WebDeploymentLog_"+ (Get-Date).ToString('yyyyMMddhhmm') +".txt"
 $DepSummaryCSV=$ConfigLocation+"\WebDeploymentSummary"+(Get-Date).ToString('yyyyMMddhhmm')+"_"+ $ENV +".csv"
 ####### Create configuration file for deployement ##########
 $ConfigFilePath =$ConfigLocation+"WebReleaseConfig.xml"
...
@( 
     ...
     $EnvConfigPath  = $ApplicationPathRoot +"\DeploymentConfig\EnvironmentConfigWeb-"+$ENV+".csv"
     ...
     Else { LogFileGen -SummaryTxt "Environment Config file is Missing or incorrect config file."
     #break;
      }
 )
 ...
 #>
  ...
  # Function for execute sql scripts. =======================================
       Function Execute-Sql{
           param($VALUE, $DbName, $ScriptFile,$excOrder)

           $LogFilePathD =$LogPath + $buildNumber+"_"+$VALUE.Replace("\" ,"-")+"_"+$DbName+"_Log.txt"
           try
           {
                [System.Reflection.Assembly]::LoadWithPartialName("Microsoft.SqlServer.Smo") | Out-Null 
                LogFileGen -SummaryTxt ( "Deployment start for :- " + $DbName + " on Server :-" +$VALUE +" SQL Script file :-" + $ScriptFile)
                Invoke-Sqlcmd -ServerInstance "$VALUE" -InputFile "$ScriptFile" -ErrorAction 'Continue' -Database "$DbName" -OutputSqlErrors:$true -Verbose *>&1  |Out-File -FilePath $LogFilePathD 
                ...
                $depStat= Import-CSV -Path $DepSummaryCSV | Where-Object {$_.VALUE -ne ""}
                ForEach($item in  $depStat) 
                { 
                    if($item.VALUE -eq $VALUE -and $item.DBNAME -eq $DbName) 
                    { 
						$item.Details = "";
...
<#
  ...
  {
    $csvHeader="BuildNumber,ENV,CLIENT,VALUE,DBNAME,Status,DBScript,DeploymentDateTime,Details"
    $csvHeader| select 'BuildNumber','ENV','CLIENT','VALUE', 'DBNAME','Status', 'DBScript','DeploymentDateTime','Details'| Export-Csv $DepSummaryCSV -NoTypeInformation
  }
  

  foreach($serv in ($EnvDetailList|Where-Object {$_.Type -eq "Server"} ) )
  {
  ...
  #unzip
        ...
        Foreach($task in ($EnvDetailList|Where-Object {$_.PARENTNODE -eq $serv.VALUE -and $_.TYPE -eq "AppPool"}))
        {
            LogFileGen -SummaryTxt "Stopping application pool " $task.VALUE
            invoke-command -ComputerName $serv.VALUE {Import-Module webadministration
                                                    Stop-WebAppPool $task.VALUE
                                                    } 
            LogFileGen -SummaryTxt "Application pool $task.VALUE stopped successfully."
        }

   
<#
        Foreach($task in ($EnvDetailList|Where-Object {$_.PARENTNODE -eq $serv.VALUE -and $_.TYPE -eq "Process"}))
        {
            LogFileGen -SummaryTxt "Stopping Windows process " $task.VALUE
            Invoke-Command -computername $task.VALUE {Stop-Process -processname $serv.VALUE -Force} -verbose 
            LogFileGen -SummaryTxt "Windows processes  $task.VALUE stopped successfully."
        }



        Foreach($task in ($EnvDetailList|Where-Object {$_.PARENTNODE -eq $serv.VALUE -and $_.TYPE -eq "Service"}))
        {
            LogFileGen -SummaryTxt "Stopping Windows Services "  $task.VALUE
            Stop-Service -InputObject $(Get-Service -ComputerName $serv.VALUE -Name $task.VALUE) -verbose
            LogFileGen -SummaryTxt "Windows Services  $task.VALUE stopped successfully."
        }

        
        Foreach($task in ($EnvDetailList|Where-Object {$_.PARENTNODE -eq $serv.VALUE -and $_.TYPE -eq "Directory"}))
        {
            LogFileGen -SummaryTxt "Deploying file to " $task.VALUE
            $destination="\\"+$serv.VALUE+"\"+$task.VALUE
            Copy-Item -Path $DropLocation\* -Destination $destination -Force
            LogFileGen -SummaryTxt "Deployment to " $task.VALUE " completed."
        }
      
        Foreach($task in ($EnvDetailList|Where-Object {$_.PARENTNODE -eq $serv.VALUE -and $_.TYPE -eq "Service"}))
        {
            LogFileGen -SummaryTxt "Starting Windows Services "  $task.VALUE
            Start-Service -InputObject $(Get-Service -ComputerName $serv.VALUE -Name $task.VALUE) -verbose
            LogFileGen -SummaryTxt "Windows Services  $task.VALUE started successfully."
        }
        #>
        ...
        Foreach($task in ($EnvDetailList|Where-Object {$_.PARENTNODE -eq $serv.VALUE -and $_.TYPE -eq "AppPool"}))
        {
            LogFileGen -SummaryTxt "Starting application pool " $task.VALUE
            invoke-command -ComputerName $serv.VALUE {Import-Module webadministration
                                                    Start-WebAppPool $task.VALUE
                                                    } 
            LogFileGen -SummaryTxt "Application pool $task.VALUE started successfully."
        }
     

  }
  <#
  foreach($db in $DatabaseList | where {$_.ScriptFile -ne "" -and $_.DatabaseName -notin ($release.Name) -and $_.ExcOrder -in ($DbList -split ",")} )
  {
           ...
           $depSum=Import-Csv $DepSummaryCSV | Where-Object {$_.VALUE -ne ""}
           if($depSum -ne $null  -and $EnvValue -ne $null){
               $depDiff=Compare-Object $EnvValue $depSum -Property VALUE,DBNAME,CLIENT,ENV  | where-object {$_.SideIndicator -eq "<="}
           }
           else
           {
               $depDiff=$EnvValue
           }
           $depDiff | Select-Object @{Name='BuildNumber';Expression={$BUILDNUMBER}},'ENV','Client','VALUE','DBNAME',@{Name='Status';Expression={'Pending'}},@{Name='DBScript';Expression={$scriptNameonly1}},@{Name='DeploymentDateTime';Expression={''}},@{Name='Details';Expression={''}}|Export-Csv $DepSummaryCSV -Append -NoTypeInformation -Force
           #$tmp=$EnvValue | select  $BUILDNUMBER,'Client','VALUE','DBNAME','Status','DBScript','DeploymentDateTime'#| Export-Csv $DepSummaryCSV -Append -NoTypeInformation
           ...
           foreach($envDB in $EnvValue)
           {
               ...
               Execute-Sql -VALUE $envDB.VALUE -DbName $envDB.DBName -ScriptFile $db.ScriptFile -excOrder $db.ExcOrder
           }
  }
