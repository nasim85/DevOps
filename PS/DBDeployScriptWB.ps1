param([string]$DropLocation="",[string]$ConfigLocation="",[string]$BUILDNUMBER="",[string]$ENV="CI",[string] $DbList="1,3,4,5,6,7,8,9,10",[string] $ReportMailTo="ReportMailTo")
<#
...
#>
 ...
 Function checkTableReuild{
           param($ReportFile)
                    ...
                    $statusTR="0"
                    [xml] $doc=(Get-Content $ReportFile)
                    if($doc.DeploymentReport.Operations.SelectNodes('*[@Name]')|Where-Object {$_.Name -in ("TableRebuild")  })
                    {
                        $statusTR="1"
