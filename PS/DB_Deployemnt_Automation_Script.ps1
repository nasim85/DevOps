param([string]$RootPath="C:\Users\300915\Desktop\RevenueCycle\R2015.4\Care\DB_Projects\ACH_DB_NonTran",[string]$DropLocation="C:\Users\300915\Desktop\RevenueCycle\R2015.4\Care\DB_Projects\ACH_DB_NonTran",[string]$BuildName=(Get-Date -format 'yyyyMMdd') +"_"+(Get-Date -format 'HHmm'),[string]$ENV="UAT",[bool]$DeployChanges=$False,[string] $DbList="13",[bool]$ExcludeSynonym=$True,[string] $ObjectList="sprocDWEligibility|vwEligibilityJoinCurrentArchive|job_Archiving_HIPAAEligibilityRequests|vwEligibilityTrace|vw_FCC_Metrics_PreRegistration|chart_FCC_Scorecard|vw_FCC_Coverage_NewSites|vw_FCC_Coverage_TEST|vw_FCC_Metrics_PreRegistration_old|vw_FCC_Scorecard |vwAhtoDialer_old|usp_PriorBalanceSheetGlobalGetAllByFacilityPatientID|USP_ChangedDatabaseLocationNotification|vw_CareAnalyticsFailures|vw_SD_LoadInfo|vw_WebExceptionLog_CARE03|vwCurrentRunningStatements|vwFileExchangeJobStatus|vwFileExchangeJobStatusMyitems|vwJobStatus|vwJobStatus2|vwNotifications |vwRegistrationProcess|vwCoverageAuditStatus|vwFileExchangeNotification|vwJobStatus|vw_Columns_MetaData|vwPayorPlans|Eligibility_CoverageAuditNotification")
#param([string]$RootPath="C:\Users\300915\Desktop\RevenueCycle\DBAutomation\SSDP\ACH_DB_Tran",[string]$DropLocation="C:\Users\300915\Desktop\RevenueCycle\DBAutomation\SSDP",[string]$BuildName=(Get-Date -format 'yyyyMMdd') +"_"+(Get-Date -format 'HHmm'),[string]$ENV="UAT",[bool]$DeployChanges=$False,[string] $DbList="15",[bool]$ExcludeSynonym=$True)
...
{  
 ...
 # Write-Host $FileContaints.GetValue($i).ToString()
           ...
           $lineSkip=22
            ...
            {
                 if (($FileContaints.GetValue($i) -match "PRINT N'Dropping" -and $FileContaints.GetValue($i+4) -match "DROP USER|DROP ROLE|DROP SCHEMA|DROP SYNONYM|DROP FULLTEXT|EXECUTE sp_droprolemember" )-or
                    ($FileContaints.GetValue($i) -match "PRINT N'Creating" -and $FileContaints.GetValue($i+4) -match "CREATE SYNONYM")
                     )
                    ...
                    else
                    {
                      if ($ObjectList -ne "0")
                            {
                                if ($FileContaints.GetValue($i) -match "PRINT N'" -and $FileContaints.GetValue($i) -notmatch $ObjectList -and $FileContaints.GetValue($i) -notmatch "PRINT N'SQLCMD mode must be enabled to successfully execute this script.';|PRINT N'The transacted portion of the database update succeeded.'|PRINT N'The transacted portion of the database update failed.'|PRINT N'Update complete.';" )
                                {
                
                               #$FileContaints.GetValue($i) |Out-File -FilePath $TargetFile -Append

                                    for ($i=$i+1; $i -lt $FileContaints.Count; $i++)
                                    {
                        
                                        if ($FileContaints.GetValue($i) -match "PRINT N'")
                                        {
                                            $FileContaints.GetValue($i) |Out-File -FilePath $TargetFile -Append
                                            break
                                        }
