param([string]$RootPath="C:\Users\300915\Desktop\RevenueCycle\R2015.6\Care\DB_Projects\ACH_DB_NonTran",[string]$DropLocation="C:\Users\300915\Desktop\RevenueCycle\R2015.6\Care\DB_Projects",[string]$BuildName=(Get-Date -format 'yyyyMMdd') +"_"+(Get-Date -format 'HHmm'),[string]$ENV="CI",[string]$DeployChanges="0",[string] $DbList="1",[bool]$ExcludeSynonym=$true)
...
Function GetOutScriptFileName([string] $DBClass)
{
    ...
    return $RootPath + "\DailyBuildScript\"+ $Env+"_"+$FileNo+"_"+$DBClass+"_"+$BuildName+".sql"
}
...
if($DBClass -eq "Tran")
    ...
    $FileContaints=$FileContaints.Replace("PRINT N'The transacted portion of the database update succeeded.'","PRINT N'Updating Gold.dbversion...'`n`n Update Gold.dbversion set BuildNumber='$buildString',BuildDate=getdate()  `n`n PRINT N'The transacted portion of the database update succeeded.'")
	}
 ...
 # Write-Host $FileContaints.GetValue($i).ToString()
            ...
            $PrintTRG = "Y"
           $lineSkip=22
            if (($FileContaints.GetValue($i) -match "PRINT N'Rename refactoring operation"))
                {
                $lineSkip=3
                }
            if (($FileContaints.GetValue($i) -match "SELECT OperationKey FROM") -and ($FileContaints.GetValue($i) -match "IF NOT EXISTS ") -and ($FileContaints.GetValue($i) -match "__RefactorLog"))
                {
                $lineSkip=1
                }
...
#                     )
                 ...
                 if (($FileContaints.GetValue($i) -match "PRINT N'Dropping" -and $FileContaints.GetValue($i+4) -match "DROP SCHEMA|DROP FULLTEXT|sp_droprolemember" )-or
                   ($FileContaints.GetValue($i) -match "PRINT N'Creating" -and $FileContaints.GetValue($i+4) -match "CREATE STATISTICS") -or
                    ($FileContaints.GetValue($i) -match "PRINT N'Rename refactoring operation|SELECT OperationKey FROM RefactorLog") -or
                    (($FileContaints.GetValue($i) -match "SELECT OperationKey FROM") -and ($FileContaints.GetValue($i) -match "IF NOT EXISTS ") -and ($FileContaints.GetValue($i) -match "__RefactorLog")) 
                     )
       ...
       Try
        {
           ...
           $UpdatedChangeScriptFile = GetOutScriptFileName -DBClass $DbProjectName
              ...
              if ((Test-Path $ProjDacpacPath) -and (Test-Path $ProfilePath ))
              {
                   ...
                   LogFileGen -SummaryTxt "Generating deployment report... "
                   ...
                   LogFileGen -SummaryTxt "Deployment report generated. "
                   ...
                   #else
                   #{
                        LogFileGen -SummaryTxt "Generating SQL change script... "
                        ...
                        LogFileGen -SummaryTxt "Change script generated. "
                   #}
