param([string]$DropLocation="",[string]$ConfigLocation="",[string]$BUILDNUMBER="",[string]$ENV="CI",[string] $DbList="1,3,4,5,6,7,8,9,10",[string] $ReportMailTo="ReportMailTo",[string] $updateReplication="0",[string] $deployTableRebuild="0")
param([string]$DropLocation="D:\TFSWebBuild\_Work\6\s",[string]$ConfigLocation="D:\RMConfig\",[string]$BUILDNUMBER="DB_NonTran_CI_R2016.6_20161208454.1",[string]$ENV="CI",[string] $DbList="1,5,9",[string] $ReportMailTo="nasimuddin@accretivehealth.com",[string] $updateReplication="0",[string] $deployTableRebuild="0")
<#
...
#>
<#
$BUILDNUMBER="DB_NonTran_CI_R2016.6_2016120280.1"
$DropLocation="C:\Users\300915\Desktop\Scripts"
$ConfigLocation="C:\Users\300915\Desktop\RMConfig\"
$deployTableRebuild="1"
$updateReplication="1"
#>
...
###################### Global Variables ########################################
 ...
 $RepConfigPath=$DropLocation +"\DeploymentTools\ReplicationConfig-"+$ENV+".csv"
...
@(            
	...
	$DatabaseList+=New-Object PsObject -property @{  DatabaseName = "DNN"; ExcOrder = "1"; ScriptFile=""; Replication = "1"; ReportFile = ""}
    #$DatabaseList+=New-Object PsObject -property @{  DatabaseName = "DNNStage"; ExcOrder = "2"  ; ScriptFile="" }
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "ClaimStatus"; ExcOrder = "3" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "CrossSiteYBFU"; ExcOrder = "4" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "Reference"; ExcOrder = "5" ; ScriptFile=""; Replication = "1"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "Global_AhtoDialer"; ExcOrder = "6" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "DataArchive"; ExcOrder = "7" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "ELIGIBILITY"; ExcOrder = "8" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "Accretive"; ExcOrder = "9" ; ScriptFile=""; Replication = "1"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "AccretiveLogs"; ExcOrder = "10" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "CrossSiteSupport"; ExcOrder = "11" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "Global_FCC_PreRegistration"; ExcOrder = "12" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "TranGLOBAL"; ExcOrder = "13" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "FileExchange"; ExcOrder = "14" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "Tran"; ExcOrder = "15" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
    $DatabaseList+=New-Object PsObject -property @{  DatabaseName = "AhCrossSite"; ExcOrder = "16" ; ScriptFile=""; Replication = "0"; ReportFile = ""}
  ...
  @(
    ...
    $dtScriptList = Get-ChildItem $SQLScriptPath |  Where-Object {$_.name -match $buildNumber  -and $_.Extension -in(".sql",".xml") }#|Select-Object -First 1
    ...
    if ($dtScriptList.Count -gt 0)
     {     
       foreach($script in $dtScriptList)
       {
           ...
           $ScriptDBName=($script.Name -split '_')[2]
           if($script.Extension -eq ".sql")
           {
               ...
               LogFileGen -SummaryTxt  "$script : updated in deployment list"
           }
           else
           {
               $listitemRep= $DatabaseList|Where-Object {$_.DatabaseName -eq $ScriptDBName}
		       if($listitemRep)
               {
               $listitemRep.ReportFile=$script.FullName
		       }
               LogFileGen -SummaryTxt  "$script : updated in deployment list"
           }
       }
        ...
        Function sendMail()
{
				...
				foreach($db in $DatabaseList | where {$_.ScriptFile -ne "" -and $_.ExcOrder -in ($DbList -split ",")} )
				{
					$att1 = new-object Net.Mail.Attachment($db.ScriptFile)
...
#>
 ...
 Function StopReplication{
           param($ReportFile,$Server,$Database)
                    $replicationList=Import-Csv $RepConfigPath | Where-Object {$_.env -eq $Env -and $_.Database -eq $Database}
                            ...
                            if($node.HasChildNodes)
                            {
                                ...
                                foreach ($filterNode in $node.ChildNodes|Where-Object {$_.Type -match "SqlTable|SqlView|SqlProcedure|SqlScalarFunction|SqlMultiStatementTableValuedFunction|SqlInlineTableValuedFunction"}) 
                                    { 
                                        ...
                                        $ArrObj=$filterNode.Value.Split(".")
                                        $objName=$ArrObj[$ArrObj.count-1]
                                        $objName=$objName.Replace("[","").Replace("]","")
                                        if(($replicationList|Where-Object{$_.Name -eq $objName -and (($Operation -in ("Drop","Alter","TableRebuild") -and $filterNode.Type -eq "SqlTable") -or ($Operation -eq "Drop"))  }))
                                        {
                                            #$Operation +":-"+$objName +" Type:-"+$filterNode.Type
                                            $SQLScript="Exec [DB_Admin].dbo.DBA_drop_replication_object '[$Server]','[$Database]','[$objName]'"
                                            $result=Sqlcmd -b -E -S "$Server" -Q "$SQLScript" -d "$Database" 

                                            LogFileGen -SummaryTxt $result 
                                            $result
                                            if($LASTEXITCODE -eq 1)
                                                {
                                                    throw $result
                                            ...
                                            Write-Host "Object removed from replication $database >> $objName"
                                            LogFileGen -SummaryTxt ( "Object removed from replication " +$Database +" >> "+ $objName)

                                           
                                        }
                                        else
                                        {
                                            $msg="Object "+ $Database+ ">>" +$objName +" not found in Replication Config file."
                                            Write-Warning $msg
 ...
 Function AddReplication{
           param($Server,$Database)
                    $replicationList=Import-Csv $RepConfigPath | Where-Object {$_.env -eq $Env -and $_.Database -eq $Database}
                            ...
                            if(($replicationList|Where-Object{$_.Name -eq $objName -and $_.Replication -eq "Yes"}))
                            {
                                #$Operation +":-"+$objName +" Type:-"+$filterNode.Type
                                $SQLScript="Exec [DB_Admin].dbo.DBA_add_replication_object '[$Server]','[$Database]','[$objName]'"
                                $result=Sqlcmd -b -E -S "$Server" -Q "$SQLScript" -d "$Database" 
                                LogFileGen -SummaryTxt $result 
                                $result
                                if($LASTEXITCODE -eq 1)
                            ...
                            {
                                $msg="Object "+ $Database+ ">>" +$objName +" not found in Replication Config file or Replication status is No."
                                Write-Warning $msg
                                LogFileGen -SummaryTxt ( $msg)
                            }
                        } 
 ...
 Function checkTableReuild{
           param($ReportFile)

                    $statusTR="0"
                    [xml] $doc=(Get-Content $ReportFile)
                    #$RebuildList=$doc.DeploymentReport.Operations.SelectNodes('*[@Name]')
                    $RebuildList=$doc.SelectSingleNode("//DeploymentReport/Operations[name()='TableRebuild']")
                    #$RebuildList=$doc.DeploymentReport.Operations.SelectNodes('*[@Name]')|Where-Object {$_.Name -in ("TableRebuild")  }
                    if($RebuildList.count -gt 0)
                    {
                        $statusTR="1"
                        $global:msgGlobal="Table Rebuild operation exists for below mentioned tables.`n Please validate code or set DeployTableRebuild pareter to 1"

                    foreach($node in ($doc.DeploymentReport.Operations.SelectNodes('*[@Name]')|Where-Object {$_.Name -in ("TableRebuild") }))
                        {

                        foreach ($filterNode in $node.ChildNodes) 
                                    { 

                                        $ArrObj=$filterNode.Value.Split(".")
                                        $objName=$ArrObj[$ArrObj.count-1]
                                        $objName=$objName.Replace("[","").Replace("]","")
                                        $global:msgGlobal=$global:msgGlobal+","+$objName
                                    }
  ...
  foreach($db in $DatabaseList | where {$_.ScriptFile -ne "" -and ($_.DatabaseName -notin ($release.Name) -or ($EnvValueTran -ne $null -and $_.ExcOrder -eq '15')) -and $_.ExcOrder -in ($DbList -split ",")} )
  {
           ...
           foreach($envDB in $EnvValue)
           {
               ...
               #check for replication and stop replication
               if($db.Replication -eq "1" -and $updateReplication -eq "1")
               {
                   StopReplication -ReportFile $db.ReportFile -Server $envDB.SERVERNAME  -Database  $envDB.DBName
               }

               $resultTR= checkTableReuild  -ReportFile $db.ReportFile 

               if($deployTableRebuild -eq "0" -and $resultTR -eq "1")
               {
                   $isSuccessded="0"
              ...
              if($db.Replication -eq "1" -and $updateReplication -eq "1")
               {
                   AddReplication -Server $envDB.SERVERNAME  -Database  $envDB.DBName
               }
           }
  }
  ...
  {
    throw "Deployment Error:$global:msgGlobal. Please check deployement report or logs for more details."
  }
