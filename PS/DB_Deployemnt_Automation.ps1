param([string]$RootPath="C:\RevenueCycle\Releases\R2016.6\Care\DB_Projects\ACH_DB_Tran",[string]$DropLocation="C:\RevenueCycle\Releases\R2016.6\Care\DB_Projects",[string]$BuildName=(Get-Date -format 'yyyyMMdd') +"_"+(Get-Date -format 'HHmm'),[string]$ENV="CI",[bool]$DeployChanges=$false,[string] $DbList="15",[bool]$ExcludeSynonym=$true)
...
IF($DbList -ne "0")
{
 ...
 #$SolPath= "$RootPath\ACH_DB_All.sln"
...
<#
if($ENV -eq "CIF" -and $DbList -like "*15*")
{
    try{
           ...
           $lineSkip=22
            ...
            if ($IncludeTran -eq $true)
            {
                 if (($FileContaints.GetValue($i) -match "PRINT N'Dropping" -and $FileContaints.GetValue($i+4) -match "DROP USER|DROP ROLE|DROP SCHEMA|DROP SYNONYM|DROP FULLTEXT|EXECUTE sp_droprolemember" )-or
                    ($FileContaints.GetValue($i) -match "PRINT N'Creating" -and $FileContaints.GetValue($i+4) -match "CREATE SYNONYM|CREATE STATISTICS") -or
            ...
            Else
            {
                if (($FileContaints.GetValue($i) -match "PRINT N'Dropping" -and $FileContaints.GetValue($i+4) -match "DROP USER|DROP ROLE|DROP SCHEMA|DROP SYNONYM|DROP FULLTEXT|EXECUTE sp_droprolemember" )-or
                    ($FileContaints.GetValue($i) -match "PRINT N'Creating" -and $FileContaints.GetValue($i+4) -match "CREATE SYNONYM|CREATE STATISTICS") -or
                    ($FileContaints.GetValue($i) -match "PRINT N'Rename refactoring operation")
                    )
                {
                    $i = $i+7
                    $SummaryTxt = $FileContaints.GetValue($i)
...
# Publish the change to target database. ========================================
	 ...
	 #if ($ENV -like "QA*" -or $ENV -like "DEV*"  -or $ENV -like "CI*")
    {
        Try
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        {
